...
public class loginNext extends HttpServlet
{
	...
	throws ServletException, IOException
	{
		...
		try
		{
			ResultSet rs=st.executeQuery("select uname, pwd from uinfo_master where uname='"+uname+"' and pwd='"+pwd+"'");
			...
			if (rs.next())
			{						
				pw.println("<h1><Welcome><br><pre>");
				pw.println("<a href=http://localhost:7001\\servlet\\AfterLogin?theuser="+uname+"&password="+pwd+">Edit</a>");
				pw.println("<a href=http://localhost:7001\\servlet\\mailServlet?theuser="+uname+"&password="+pwd+">Mail</a>");
				pw.println("<a href=http://localhost:7001\\servlet\\BidItems?uname="+uname+"&pwd="+pwd+">Bid Items</a>");
				pw.println("<a href=http://localhost:7001\\servlet\\SoldItems?uname="+uname+"&pwd="+pwd+">Sold Items</a>");
				pw.println("</pre></h3><br><br><br>");
