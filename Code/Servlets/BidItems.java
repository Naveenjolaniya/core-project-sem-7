...
public class BidItems extends HttpServlet
{
	String uname, pwd;
	Statement st;
	Connection con;
	...
	throws ServletException, IOException
	{
		...
		try
		{
			...
			pw.println("<center>");	
			rs = st.executeQuery("select item_master.itemid from item_master, bidding_info where bidding_info.bidderid='"+uname+"' and item_master.itemid=bidding_info.itemid union select itemid from mailinfo where paid='NO' and bidderid='"+uname+"'");
			if(rs.next())
			{
				String iid = rs.getString(1);
				pw.println("<h1>You bid for</h1><br>");
				pw.println("<a href=http://localhost:7001/servlet/ItemDetails2?id="+iid+">"+iid+"</a><br>");
				while(rs.next())
				{
					iid = rs.getString(1);
					pw.println("<a href=http://localhost:7001/servlet/ItemDetails2?id="+iid+">"+iid+"</a><br>");
