...
public class DelItemCat extends HttpServlet
{
	...
	public void doGet(HttpServletRequest req,HttpServletResponse res) throws ServletException,IOException
	{
		...
		pw.println("function chk()");
		pw.println(" {			var i, res; 	res=0;");
		pw.println(" for(i=0;i<items.length;i++)");
		pw.println("{if(items.item(i).checked==true)");
		pw.println("{ 	res=1;	break; 	} }");
		pw.println(" 		if(res==0) 	alert(' Select a category')");
		pw.println(" 	else 			items.submit(); 	}");
		pw.println(" </script>");
		...
		pw.println("<form name=items method =post action='http://localhost:7001/servlet/ItemDel'>");
		...
		pw.println("<tr><td><input type=checkbox name="+rs.getString(1)+">"+cat);
		}
		pw.println("<center><input type=button value='Delete' onclick=chk()> </form>");
		...
		pw.println("<A href=http://localhost:7001/servlet/NewToday>New Items</A>");		pw.println("<A href=http://localhost:7001/servlet/EndToday>Closing Items</A>");
