...
public class showMail extends HttpServlet
{
	...
	throws ServletException, IOException
	{
		...
		{				
			...
			ResultSet rs = st.executeQuery("select im.itemname, biddate, bamt from bidding_info bi, item_master im where im.itemid='" + itemid + "' and bidderid='"+uname+"' and bamt=(select max(bamt) from bidding_info where itemid='" + itemid + "')");
			rs.next();
			pw.println("<form method=get action=http://localhost:7001/servlet/Pay>");
			pw.println("<input type=hidden name=itemid value="+itemid+">");
			pw.println("<input type=hidden name=theuser value="+uname+">");
			pw.println("<Table align=center><tr><td align=right><b>Item name : </td>");
			pw.println("<td><b>"+rs.getString(1)+"</td></tr>");
			pw.println("<td align=right><b>Bid date : </td>");
			pw.println("<td><b>"+rs.getString(2)+"</td></tr>");
			pw.println("<td align=right><b>Bid amount : </td>");
			String bamt = rs.getString(3);
			pw.println("<td><b>"+bamt+"</b></td></tr></table>");
			pw.println("<center><input type=button value=Back onClick=history.back()>");
			pw.println("<input type=button value=Drop onclick=\"navigate('http://localhost:7001/servlet/DropServlet?userid="+uname+"&itemid=" + itemid+"&bamt="+bamt+"')\">");
			pw.println("<input type=submit value=Pay></form>");
