...
public class mailServlet extends HttpServlet
{
	...
	{
		uname=request.getParameter("theuser");
		...
		{				
			...
			int written=0;
			ResultSet rs2=null;
			ResultSet rs = st.executeQuery("select bid.itemid from bidding_info bid, item_master im where bidderid='"+uname+"' and enddate-sysdate<0 and im.itemid=bid.itemid group by bidderid,bid.itemid having(max(bamt))=(select max(bamt) from bidding_info where bid.itemid=itemid)");
			if(rs.next())
			{
				rs2 = st.executeQuery("select bid.itemid from bidding_info bid, item_master im where bidderid='"+uname+"' and enddate-sysdate<0 and im.itemid=bid.itemid group by bidderid,bid.itemid having(max(bamt))=(select max(bamt) from bidding_info where bid.itemid=itemid)");
				while(rs2.next())
				{
					String iid = rs2.getString(1);
					ResultSet rs1 = st1.executeQuery("select * from mailinfo where bidderid='"+uname+"' and itemid='"+iid+"'");
					if(rs1.next())
					{
						rs1 = st1.executeQuery("select * from mailinfo where bidderid='"+uname+"' and itemid='"+iid+"' and paid='NO'");
						if(rs1.next())
						{
							found=1;
							if(written==0)
							{
								pw.println("<h1>U bid for</h1><br>");
								written=1;
							}
							pw.println("<a href=http://localhost:7001/servlet/showMail?theuser="+uname+"&password="+pwd+"&itemid="+iid+">"+iid+"</a><br>");
						}
					}
					else
					{
						...
						st1.executeUpdate("insert into mailinfo	values('"+uname+"','"+iid+"','NO')");
						pw.println("<a href=http://localhost:7001/servlet/showMail?theuser="+uname+"&password="+pwd+"&itemid="+iid+">"+iid+"</a><br>");
					}
				}
			}
			...
			{
				written=1;
				pw.println("<h1>No recent bids</h1>");						
			}
			if(found==0 && written==0)
				pw.println("<h1>No recent bids</h1>");
			rs=st.executeQuery("select item_master.itemid from item_master, mailinfo where paid='YES' and item_master.sellerid='"+uname+"' and item_master.itemid=mailinfo.itemid");
			if(rs.next())
			{
				pw.println("<h1>Your items which were sold<br></h1>");
				pw.println("<h3>"+rs2.getString(1)+"</h3><br>");
				while(rs.next())
					pw.println("<h3>"+rs2.getString(1)+"</h3><br>");
			}
			else	
			{
				pw.println("<h3>No sold items</h3>");
			}
			pw.println("<br><br><input type=button value=Back onclick=history.back()><br><br><br><br>");
